      ...
      <nav className={`lx-nav ${scrollY>60?"lx-nav--solid":""}`}>
        <div className="lx-nav__in">
          <div className="lx-logo" onClick={() => window.scrollTo({top:0,behavior:"smooth"})}>
            <span className="lx-logo__s">☕</span><span className="lx-logo__rest" style={{fontSize:11,letterSpacing:2}}>CAFÉ DON BERNA</span>
          </div>
  ...
  {icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#4CAF50" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,t:"Propietario",v:"Sebastian Castaño Palacio"}
      ...
      <footer className="lx-footer">
        <div className="lx-footer__in">
          <div>
            <div className="lx-logo" style={{marginBottom:12}}><span className="lx-logo__s">☕</span><span className="lx-logo__rest" style={{fontSize:11,letterSpacing:2}}>CAFÉ DON BERNA</span></div>
      ...
      {modal === "auth" && (
        <div className="lx-modal-mask" onClick={() => setModal(null)}>
          <div className="lx-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="lx-modal__brand"><span className="lx-logo__s">☕</span><span className="lx-modal__bname" style={{fontSize:13,letterSpacing:2}}>CAFÉ DON BERNA</span></div>
              ...
              <form className="lx-form" onSubmit={handleRegister}>
                ...
                <div className="lx-form__2">
                  <div className="lx-field"><label>Departamento</label>
                    <select value={regData.departamento} onChange={e=>setRegData({...regData,departamento:e.target.value,municipio:'Medellín'})}>
                      <option>Antioquia</option><option>Bogotá D.C.</option><option>Valle del Cauca</option><option>Cundinamarca</option><option>Atlántico</option><option>Bolívar</option><option>Santander</option><option>Córdoba</option><option>Nariño</option><option>Risaralda</option><option>Tolima</option><option>Huila</option><option>Cauca</option>
                    </select>
                  </div>
                  <div className="lx-field"><label>Municipio</label>
                    <select value={regData.municipio} onChange={e=>setRegData({...regData,municipio:e.target.value})}>
                      {regData.departamento==='Antioquia' && <><option>Medellín</option><option>Bello</option><option>Itagüí</option><option>Envigado</option><option>Sabaneta</option><option>Rionegro</option></>}
                      {regData.departamento==='Bogotá D.C.' && <option>Bogotá</option>}
                      {regData.departamento==='Valle del Cauca' && <><option>Cali</option><option>Buenaventura</option><option>Palmira</option></>}
                      {regData.departamento==='Cundinamarca' && <><option>Soacha</option><option>Facatativá</option><option>Zipaquirá</option></>}
                      {regData.departamento==='Atlántico' && <><option>Barranquilla</option><option>Soledad</option></>}
                      {regData.departamento==='Bolívar' && <><option>Cartagena</option><option>Magangué</option></>}
                      {regData.departamento==='Santander' && <><option>Bucaramanga</option><option>Floridablanca</option></>}
                      {regData.departamento==='Córdoba' && <><option>Montería</option><option>Lorica</option></>}
                      {regData.departamento==='Nariño' && <><option>Pasto</option><option>Tumaco</option></>}
                      {regData.departamento==='Risaralda' && <><option>Pereira</option><option>Dosquebradas</option></>}
                      {regData.departamento==='Tolima' && <><option>Ibagué</option><option>Espinal</option></>}
                      {regData.departamento==='Huila' && <><option>Neiva</option><option>Pitalito</option></>}
                      {regData.departamento==='Cauca' && <><option>Popayán</option><option>Santander de Quilichao</option></>}
                    </select>
                  </div>
                </div>
                ...
                {regData.municipio === 'Medellín' && (
                  <div className="lx-field">
                    <label>Comuna <span style={{fontSize:11,color:'#aaa',fontWeight:400}}>(servicio disponible solo en comunas 8 y 9)</span></label>
                    <select value={regData.comuna} onChange={e=>setRegData({...regData,comuna:e.target.value})}
                      style={{borderColor: regData.comuna && regData.comuna !== 'Comuna 8 - Villa Hermosa' && regData.comuna !== 'Comuna 9 - Buenos Aires' ? '#E53935' : ''}}>
                      ...
                      <option value="Comuna 12 - La América">Comuna 12 - La América</option>
                    ...
                    {regData.comuna && regData.comuna !== 'Comuna 8 - Villa Hermosa' && regData.comuna !== 'Comuna 9 - Buenos Aires' && (
                      <p style={{fontSize:12,color:'#E53935',marginTop:4,fontWeight:600}}>
                        ⚠ Lo sentimos, el servicio de domicilios solo está disponible para las comunas 8 y 9. Puedes visitarnos en nuestro punto físico.
                      </p>
                    )}
          ...
          <div className="lx-modal-mask" onClick={() => setModal(null)}>
            <div className="lx-modal lx-perfil-modal" onClick={e => e.stopPropagation()} style={{maxWidth:520,width:'95%',maxHeight:'90vh',overflowY:'auto',padding:'32px 28px'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:24}}>
                ...
                <div>
                  <div style={{fontWeight:700,fontSize:18,color:'var(--ink)'}}>{clienteSession.nombre}</div>
                  <div style={{fontSize:13,color:'var(--muted)'}}>{cliente?.correo}</div>
                  <div style={{fontSize:11,color:'#10b981',fontWeight:600,marginTop:2}}>● Cliente activo</div>
                </div>
              </div>
              <div style={{display:'flex',borderBottom:'2px solid #f0f0f0',marginBottom:24,gap:0}}>
                {[["info","👤 Mi perfil"],["historial","📋 Historial"],["editar","✏️ Editar datos"]].map(([tab,label]) => (
                  <button key={tab} onClick={() => { setPerfilTab(tab); setEditError(""); setEditSuccess(""); }} style={{flex:1,padding:'10px 0',border:'none',background:'none',cursor:'pointer',fontSize:13,fontWeight:600,color:perfilTab===tab?'var(--gold)':'var(--muted)',borderBottom:perfilTab===tab?'2px solid var(--gold)':'2px solid transparent',marginBottom:-2,transition:'all .2s'}}>{label}</button>
                ))}
              </div>
              {perfilTab === "info" && (
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  {[{icon:'📧',label:'Correo',val:cliente?.correo},{icon:'📞',label:'Teléfono',val:cliente?.telefono||'—'},{icon:'📍',label:'Dirección',val:cliente?.direccion||'—'},{icon:'🗺️',label:'Ubicación',val:cliente?.municipio&&cliente?.departamento?`${cliente.municipio}, ${cliente.departamento}`:'—'},{icon:'🪪',label:'Documento',val:cliente?.tipoDoc&&cliente?.numeroDoc?`${cliente.tipoDoc}: ${cliente.numeroDoc}`:'—'},{icon:'📅',label:'Miembro desde',val:cliente?.fechaRegistro?new Date(cliente.fechaRegistro).toLocaleDateString('es-CO',{year:'numeric',month:'long',day:'numeric'}):'—'}].map(({icon,label,val}) => (
                    <div key={label} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'#fafafa',borderRadius:10,border:'1px solid #f0f0f0'}}>
                      <span style={{fontSize:18,width:28,textAlign:'center'}}>{icon}</span>
                      <div><div style={{fontSize:11,color:'var(--muted)',fontWeight:600,textTransform:'uppercase',letterSpacing:.5}}>{label}</div><div style={{fontSize:14,color:'var(--ink)',fontWeight:500,marginTop:1}}>{val}</div></div>
                    </div>
                  ))}
                  ...
                  {pedidosCliente.length === 0 ? (
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:12}}>
                      ...
                      {pedidosCliente.map(p => (
                        <div key={p.id} style={{border:'1px solid #eee',borderRadius:12,padding:'14px 16px',background:'#fafafa'}}>
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                            <span style={{fontWeight:700,fontSize:13,color:'var(--ink)'}}>Pedido #{p.id}</span>
                            <span style={{fontSize:11,fontWeight:700,padding:'3px 10px',borderRadius:100,background:estadoColor[p.estado]+'20',color:estadoColor[p.estado]}}>{estadoLabel[p.estado]||p.estado}</span>
                          </div>
                          {Array.isArray(p.productos)&&p.productos.length>0&&<div style={{fontSize:12,color:'var(--muted)',marginBottom:6}}>{p.productos.map(x=>`${x.nombre||x} x${x.cantidad||1}`).join(' · ')}</div>}
                          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                            <span style={{fontSize:12,color:'var(--muted)'}}>{p.fechaCreacion?new Date(p.fechaCreacion).toLocaleDateString('es-CO',{day:'2-digit',month:'short',year:'numeric'}):''}{p.hora?` · ${p.hora}`:''}</span>
                            <span style={{fontWeight:700,fontSize:14,color:'var(--gold)'}}>{fmt2(p.total)}</span>
                          </div>
                        </div>
                      ))}
                    </div>
