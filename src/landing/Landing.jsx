              ...
              {toppings.map(t => {
                ...
                return (
                  <button key={t.id} onClick={() => toggleTop(t)} style={{padding:'8px 16px', borderRadius:100, fontSize:12, fontWeight:600, cursor:'pointer', border:`1.5px solid ${sel?'var(--gold)':'rgba(14,12,7,.14)'}`, background:sel?'rgba(76,175,80,0.15)':'var(--bg-hover)', color:sel?'var(--lx-text)':'var(--lx-muted)', transition:'all .2s'}}>
                    {t.nombre}
                    {t.precio > 0 ? <span style={{color:'var(--gold)', marginLeft:4}}>+{fmt(t.precio)}</span> : null}
...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    adicionesService.getAll()
      ...
      .catch(() => setAdiciones([]));
    // Fichas técnicas + insumos reales (antes esto se leía de localStorage
    // con claves que nunca se llenaban, así que la disponibilidad "real"
    // siempre daba true sin importar el stock actual).
    fichasTecnicasService.getAll()
      .then(d => setFichasTec(Array.isArray(d) ? d : []))
      .catch(() => setFichasTec([]));
    insumosService.getAll()
      .then(d => setInsumosStock(Array.isArray(d) ? d : []))
      .catch(() => setInsumosStock([]));
  }, []);
  // ─────────────────────────────────────────────────────────

  // Máximo de unidades vendibles de un producto según el inventario real:
  // para cada insumo de su ficha técnica, cuántas unidades del producto
  // alcanza ese insumo (stock ÷ cantidad por unidad) — el insumo más
  // limitante define el máximo. Sin ficha técnica, no hay límite conocido.
  const maxDisponible = (prodId) => {
    const ficha = fichasTec.find(f => String(f.id_producto) === String(prodId) && f.estado !== false);
    if (!ficha || !Array.isArray(ficha.insumos) || ficha.insumos.length === 0) return Infinity;
    let max = Infinity;
    ficha.insumos.forEach(receta => {
      const insumo = insumosStock.find(i => String(i.id) === String(receta.id_insumo));
      if (!insumo) return;
      const porUnidad = Number(receta.cantidad) || 1;
      const disponibles = Math.floor(Number(insumo.stockActual) / porUnidad);
  ...
  const updateQty  = (cartKey,d,prodId) => setCart(prev => prev.map(i => {
    ...
    if (d > 0 && nueva > max) {
      showToast(`No es posible agregar más unidades. El máximo disponible para este producto es ${max} según el inventario actual.`);
      return { ...i, qty: max };
    }
    return { ...i, qty: nueva };
  }).filter(i=>i.qty>0));
