...
const ClientesPage = () => {
  ...
  const { clientes, remove, toggleEstado } = useClientes();
    ...
    <Layout>
      <div className="insumos-root">
        ...
        <div className="page-header">
          ...
          <p className="page-subtitle">{clientes.length} cliente{clientes.length!==1?'s':''} registrado{clientes.length!==1?'s':''} desde la landing page</p>
        </div>
        ...
        <div className="insumos-toolbar">
          ...
          <span style={{fontSize:13,color:'#888',marginLeft:'auto'}}>{shown.length} cliente{shown.length!==1?'s':''}</span>
        </div>
        ...
        <div className="insumos-card">
          {shown.length === 0 ? (
            <div className="empty-state">
              ...
              <p>{query ? 'Intenta otro término.' : 'Cuando alguien se registre en la landing page, aparecerá aquí automáticamente.'}</p>
            </div>
          ) : (
                    ...
                    <tr key={c.id}>
                      <td>
                        <div style={{display:'flex',alignItems:'center',gap:10}}>
                          <div style={{width:36,height:36,borderRadius:'50%',background:'#E8F5E9',color:'#2E7D32',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,fontSize:14,flexShrink:0}}>
                            {c.nombre.charAt(0).toUpperCase()}
                          </div>
                          <div>
                            <div className="td-nombre">{c.nombre}</div>
                            <div style={{fontSize:12,color:'#aaa'}}>{c.correo}</div>
                          </div>
                        </div>
                      </td>
                      <td style={{color:'#888',fontSize:13}}>{c.telefono||'—'}</td>
                      <td style={{color:'#888',fontSize:13,maxWidth:160,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.direccion||'—'}</td>
                      <td>
                        <button
                          className={`toggle-btn ${c.estado ? 'toggle-on' : 'toggle-off'}`}
                          onClick={() => toggleEstado(c.id)}
                          title={c.estado ? 'Activo' : 'Inactivo'}
                        >
                          <span className="toggle-thumb"/>
                        </button>
                      </td>
                      <td style={{color:'#888',fontSize:13}}>{fmt(c.fechaRegistro)}</td>
                      <td>
                        <div className="actions-group">
                          <button className="btn-ver" title="Ver" onClick={() => navigate(`/admin/clientes/ver/${c.id}`)}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                          </button>
                          <button className="btn-editar" title="Editar" onClick={() => navigate(`/admin/clientes/editar/${c.id}`)}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                          </button>
                          <button className="btn-anular" title="Anular" onClick={() => { setDeleteError(''); setDeleteTarget(c); }}>
                            ✕ Anular
                          </button>
                        </div>
                      </td>
        ...
        {deleteTarget && (
          <div className="modal-overlay" onClick={() => setDeleteTarget(null)}>
            <div className="modal-box" onClick={e => e.stopPropagation()}>
              ...
              {deleteError && <div style={{background:'#FFEBEE',color:'#B71C1C',padding:'8px 12px',borderRadius:8,marginBottom:16,fontSize:13}}>⚠ {deleteError}</div>}
