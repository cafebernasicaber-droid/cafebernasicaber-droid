...
function DetailModal({ order, onClose }) {
  ...
  // Buscar fichas técnicas para cada producto del pedido
  const productosConFicha = productos
    .map(it => ({ ...it, ficha: fichasTecnicasService.getByProducto(Number(it.id) || Number(it.id_producto)) }))
    .filter(it => it.ficha && it.ficha.estado);
