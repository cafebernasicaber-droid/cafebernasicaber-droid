      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'20px 24px 16px',borderBottom:'1px solid rgba(255,255,255,.07)' }}>
          <div style={{ display:'flex',alignItems:'center',gap:12 }}>
            ...
            <div>
              <div style={{ fontWeight:800,fontSize:16,color:'#f0ece4' }}>Compra #{compra.id}</div>
        ...
        <div style={{ padding:'20px 24px' }}>
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14 }}>
            ...
            <div style={{ background:'#2a2a2a',borderRadius:12,padding:'16px 18px',border:'1px solid rgba(255,255,255,.07)' }}>
              <div style={{ fontSize:11,fontWeight:700,color:'#a09880',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:12 }}>Resumen</div>
              {[
              ...
              {compra.observaciones && (
                <div style={{ marginTop:10,fontSize:12,color:'#a09880' }}>
          ...
          <div style={{ background:'#2a2a2a',borderRadius:12,padding:'14px 18px',border:'1px solid rgba(255,255,255,.07)',marginBottom:14 }}>
            <div style={{ fontSize:11,fontWeight:700,color:'#a09880',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:10 }}>Detalle de Insumos</div>
            {compra.items && compra.items.length > 0 ? (
              <table style={{ width:'100%',borderCollapse:'collapse',fontSize:13 }}>
                <thead>
                  <tr style={{ borderBottom:'2px solid rgba(255,255,255,.1)' }}>
                    {['Insumo','Unidad','Cantidad','Precio unit.','Subtotal'].map(h => (
                      <th key={h} style={{ padding:'6px 8px',textAlign:'left',fontWeight:700,color:'#a09880',fontSize:12 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {compra.items.map((item, idx) => (
                    <tr key={idx} style={{ borderBottom:'1px solid rgba(255,255,255,.05)' }}>
                      <td style={{ padding:'7px 8px',fontWeight:600,color:'#f0ece4' }}>{item.insumo}</td>
                      <td style={{ padding:'7px 8px',color:'#a09880' }}>{item.unidad || '—'}</td>
                      <td style={{ padding:'7px 8px',color:'#f0ece4' }}>{item.cantidad}</td>
                      <td style={{ padding:'7px 8px',color:'#f0ece4' }}>{formatCOP(item.precioUnitario)}</td>
                ...
                <tfoot>
                  <tr style={{ borderTop:'2px solid rgba(255,255,255,.1)',background:'rgba(255,255,255,.04)' }}>
                    <td colSpan="4" style={{ padding:'8px',fontWeight:700,color:'#a09880',fontSize:13 }}>Total</td>
                    <td style={{ padding:'8px',fontWeight:800,color:'#FFCC80',fontSize:15 }}>{formatCOP(compra.total)}</td>
                  </tr>
                </tfoot>
              </table>
            ) : (
              <p style={{ color:'#a09880',fontSize:13,margin:0 }}>Sin insumos registrados.</p>
            )}
          </div>
      ...
      <div className="insumos-root">
        ...
        <div className="insumos-toolbar">
          ...
          <div style={{ display:'flex',gap:8 }}>
            <button
              style={{ padding:'10px 18px',borderRadius:10,border:'1px solid rgba(255,255,255,.12)',background:'#2a2a2a',color:'#a09880',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:6 }}
                ...
                <tbody>
                  {displayed.map(c => (
                    <tr key={c.id}>
                      ...
                      <td>
                        <span style={{ padding:'3px 10px',borderRadius:20,fontSize:11,fontWeight:700,background:'#E8F5E9',color:'#2E7D32',border:'1px solid #A5D6A7' }}>
                          Activa
                        </span>
                      </td>
                      <td>
