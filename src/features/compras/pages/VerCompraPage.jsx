...
const VerCompraPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const found = await comprasService.getById(id);
        if (!activo) return;
  ...
  const handleAnular = async () => {
    ...
    try {
      const ok = await comprasService.anular(id, motivo.trim());
      if (ok) {
      ...
      showError(err.message || 'No se pudo anular la compra.');
    }
        ...
        <div className="ver-grid">
          <div className="ver-card">
            ...
            <div className="ver-fields">
              ...
              <div className="ver-field">
                <span className="vf-label">Comprobante de compra</span>
                {compra.comprobante_url ? (
                  <span className="vf-value" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <a href={compra.comprobante_url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-green,#4CAF50)', fontWeight: 600 }}>
                      Ver comprobante
                    </a>
                    {compra.comprobante_verificado && (
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: 'rgba(76,175,80,0.15)', color: '#4CAF50' }}>
                        ✓ Total verificado
                      </span>
                    )}
                  </span>
                ) : (
                  <span className="vf-value" style={{ color: 'var(--text-muted)' }}>Sin comprobante</span>
                )}
              </div>
            </div>
          </div>
