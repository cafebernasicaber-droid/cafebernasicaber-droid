  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" style={{maxWidth:520,textAlign:'left',padding:'32px 36px'}} onClick={e=>e.stopPropagation()}>
        ...
        <p style={{fontSize:13,color:'#888',marginBottom:20}}>{inicial?`Modificando: ${inicial.nombre}`:'Agrega una categoría al menú'}</p>
          ...
          <div className="mod-form-group">
            ...
            {!form.imagen ? (
              <div
                ...
                <p style={{margin:'3px 0 10px',fontSize:12,color:'#aaa'}}>o haz clic para seleccionarla</p>
                <span style={{display:'inline-block',padding:'5px 16px',background:'#4CAF50',color:'white',borderRadius:20,fontSize:12,fontWeight:600}}>
                  Seleccionar imagen
                </span>
...
export default function CategoriasPage() {
  const [cats, setCats] = useState(() => categoriasService.getAll());
  ...
  const refresh = () => setCats(categoriasService.getAll());
  const showOk = msg => { setSuccess(msg); setTimeout(()=>setSuccess(''),3000); };

  const shown = query.trim() ? categoriasService.search(query) : cats;
  ...
  return (
    <Layout>
      <div className="insumos-root">
        ...
        {deleteError && (
          <div className="toast" style={{background:'#FFEBEE',color:'#B71C1C',border:'1px solid #FFCDD2',top:24,right:28,position:'fixed',zIndex:2000,display:'flex',alignItems:'center',gap:10,padding:'14px 20px',borderRadius:10,fontSize:13,fontWeight:600,boxShadow:'0 8px 24px rgba(0,0,0,0.15)',maxWidth:420,animation:'slideInRight 0.3s ease'}}>
            ⚠️ {deleteError}
          </div>
        )}
        {modal && (
        ...
        <div className="insumos-toolbar">
          ...
          <span style={{fontSize:13,color:'#888',marginLeft:'auto'}}>{shown.length} categoría{shown.length!==1?'s':''}</span>
                    ...
                    <tr key={c.id}>
                      <td>
                        {c.imagen
                          ...
                          : <div style={{width:40,height:40,borderRadius:8,background:'#f5f5f5',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20}}>📂</div>
                        }
                      </td>
                      <td className="td-nombre">{c.nombre}</td>
                      <td style={{fontSize:13,color:'#666',maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.descripcion||'—'}</td>
                      <td>
                        <button className={`toggle-btn ${c.estado==='Activo'?'toggle-on':'toggle-off'}`}
                          onClick={()=>{categoriasService.toggleEstado(c.id);refresh();}} title={c.estado}>
                          <span className="toggle-thumb"/>
                        </button>
                      </td>
                      <td style={{fontSize:13,color:'#888'}}>{fmt(c.fechaCreacion)}</td>
                      <td>
                ...
                {paginated.map(c => (
                  <div className="prod-card" key={c.id}>
                    ...
                    <div className="prod-card__body">
                      ...
                      <div className="prod-card__cat" style={{fontSize:12,color:'#888',marginTop:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.descripcion||'Sin descripción'}</div>
                      <div className="prod-card__price" style={{fontSize:12,color:'#aaa'}}>{fmt(c.fechaCreacion)}</div>
                    </div>
                    <div className="prod-card__foot">
                      <button className={`toggle-btn ${c.estado==='Activo'?'toggle-on':'toggle-off'}`} style={{transform:'scale(0.85)'}} onClick={()=>{categoriasService.toggleEstado(c.id);refresh();}}><span className="toggle-thumb"/></button>
                      <div className="actions-group">
