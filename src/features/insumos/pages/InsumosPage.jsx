  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        background:'#1a1a1a', borderRadius:18, width:'100%', maxWidth:640,
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'20px 24px 16px', borderBottom:'1px solid rgba(255,255,255,.07)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              <div style={{ fontWeight:800, fontSize:16, color:'#f0ece4' }}>{insumo.nombre}</div>
          ...
          <button onClick={onClose} style={{ width:34,height:34,borderRadius:'50%',border:'none',background:'rgba(255,255,255,.08)',color:'#a09880',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0 }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        <div style={{ padding:'20px 24px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
            <div style={{ background:'#2a2a2a',borderRadius:12,padding:'16px 18px',border:'1px solid rgba(255,255,255,.07)' }}>
              <div style={{ fontSize:11,fontWeight:700,color:'#a09880',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:12 }}>Información General</div>
              {[
                ...
                ['Estado',
                  <div style={{ display:'flex',alignItems:'center',gap:8 }}>
                    ...
                    <span style={{ fontSize:13,fontWeight:600,color:insumo.estado?'#81C784':'#888' }}>{insumo.estado?'Activo':'Inactivo'}</span>
                  </div>
                ],
              ].map(([label, val]) => (
                <div key={label} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,.05)',fontSize:13 }}>
                  <span style={{ color:'#a09880',fontWeight:600 }}>{label}</span>
                  <span style={{ color:'#f0ece4',fontWeight:500 }}>{val}</span>
                </div>
              ))}
            </div>
            <div style={{ background:'#2a2a2a',borderRadius:12,padding:'16px 18px',border:'1px solid rgba(255,255,255,.07)' }}>
              <div style={{ fontSize:11,fontWeight:700,color:'#a09880',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:12 }}>Inventario & Precio</div>
              {[
                ...
                ['Último precio pagado', insumo.precioUnitario ? <span style={{ fontWeight:700,color:'#FFCC80' }}>{formatCOP(insumo.precioUnitario)}</span> : <span style={{ color:'#666' }}>Sin compras aún</span>],
              ].map(([label, val]) => (
                <div key={label} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,.05)',fontSize:13 }}>
                  <span style={{ color:'#a09880',fontWeight:600 }}>{label}</span>
                  <span style={{ color:'#f0ece4',fontWeight:500 }}>{val}</span>
          ...
          <div style={{ background:'#2a2a2a',borderRadius:12,padding:'14px 18px',border:'1px solid rgba(255,255,255,.07)',marginBottom:14 }}>
            <div style={{ fontSize:11,fontWeight:700,color:'#a09880',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:6 }}>Descripción</div>
            <p style={{ fontSize:13,color:'#c5bfb5',lineHeight:1.6,margin:0 }}>{insumo.descripcion || 'Sin descripción registrada.'}</p>
            <div style={{ marginTop:10,fontSize:12,color:'#666' }}>Registrado: {formatDate(insumo.fechaCreacion)}</div>
          </div>
          {!stockOk && (
...
function ModalFormInsumo({ insumo, onCreate, onUpdate, onClose }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{
        background:'#1a1a1a',borderRadius:18,width:'100%',maxWidth:680,
        ...
        <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'20px 24px 16px',borderBottom:'1px solid rgba(255,255,255,.07)' }}>
          <div style={{ display:'flex',alignItems:'center',gap:12 }}>
            ...
            <div>
              <div style={{ fontWeight:800,fontSize:15,color:'#f0ece4' }}>{isEdit?'Editar insumo':'Agregar insumo'}</div>
              <div style={{ fontSize:12,color:'#a09880' }}>{isEdit?`Modificando: ${insumo.nombre}`:'Completa los campos para registrar un nuevo insumo'}</div>
            </div>
          </div>
          <button onClick={onClose} style={{ width:34,height:34,borderRadius:'50%',border:'none',background:'rgba(255,255,255,.08)',color:'#a09880',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0 }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                      ...
                      <tr key={ins.id}>
                        ...
                        <td className="td-stock">
                          <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                            ...
                            {sinStock && (
                              <span style={{ display:'inline-flex',alignItems:'center',gap:3,padding:'2px 8px',borderRadius:20,fontSize:11,fontWeight:700,background:'#FFEBEE',color:'#C62828',border:'1px solid #EF9A9A' }}>
                                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                                Sin stock
                              </span>
                            )}
                            {stockBajo && (
                              <span style={{ display:'inline-flex',alignItems:'center',gap:3,padding:'2px 8px',borderRadius:20,fontSize:11,fontWeight:700,background:'#FFF3E0',color:'#E65100',border:'1px solid #FFCC80' }}>
                        ...
                        <td>
                          <div className="actions-group">
                            ...
                            <button className="btn-accion btn-accion-eliminar" title="Eliminar"
