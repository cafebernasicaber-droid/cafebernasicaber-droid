      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding:'20px 24px' }}>
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14 }}>
            ...
            {/* Ubicación */}
            <div style={{ background:'var(--bg-surface-3)',borderRadius:12,padding:'16px 18px',border:'1px solid var(--border)' }}>
              <div style={{ fontSize:11,fontWeight:700,color:'var(--text-secondary)',textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:12 }}>Ubicación y Categoría</div>
              {[
          ...
          <div style={{ display:'flex',justifyContent:'flex-end',gap:8 }}>
            ...
            <button onClick={onEliminar} style={{ padding:'10px 18px',background:'linear-gradient(135deg,#E53935,#B71C1C)',border:'none',borderRadius:10,color:'white',fontSize:13,fontWeight:700,cursor:'pointer' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>Stop
            </button>
            <button className="btn-confirm-primary" onClick={onEditar}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg> Editar proveedor</button>
          </div>
        </div>
      </div>
    </div>
...
const ProveedoresPage = () => {
  ...
  const handleSearch = (e) => {
    ...
    else { setFiltered(proveedores.filter(p => p.nombre?.toLowerCase().includes(val.toLowerCase()) || p.nit?.toLowerCase().includes(val.toLowerCase()) || p.correo?.toLowerCase().includes(val.toLowerCase()))); }
  };
  const clearSearch = () => { setQuery(''); setFiltered(null); searchRef.current?.focus(); };

  const openDeleteTarget = (p) => {
    const insumos     = [];
    const tieneCompras = false;
  ...
  const handleDelete = () => {
    ...
    setFiltered(query.trim() ? proveedores.filter(p => p.nombre?.toLowerCase().includes(query.toLowerCase()) || p.nit?.toLowerCase().includes(query.toLowerCase()) || p.correo?.toLowerCase().includes(query.toLowerCase())) : null);
    let msg = `Proveedor "${deleteTarget.nombre}" anulado correctamente`;
  ...
  const handleToggle = (id) => {
    toggleEstado(id);
    if (query.trim()) setFiltered(proveedores.filter(p => p.nombre?.toLowerCase().includes(query.toLowerCase()) || p.nit?.toLowerCase().includes(query.toLowerCase()) || p.correo?.toLowerCase().includes(query.toLowerCase())));
              ...
              {deleteInfo.tieneCompras ? (
                <>
                  <p style={{ color:'#B71C1C',fontWeight:600 }}>
                    ⛔ No se puede anular: "{deleteTarget.nombre}" tiene compras activas. Inactívalo en su lugar.
                  </p>
                ...
                <>
                  {deleteInfo.insumos.length > 0 && (
                    <div style={{ background:'rgba(230,115,0,0.10)',border:'1px solid rgba(230,115,0,0.28)',borderRadius:8,padding:'10px 14px',marginBottom:12,fontSize:13,color:'#E65100' }}>
                      ⚠ También se anularán los siguientes insumos asociados:
        ...
        <div className="insumos-toolbar">
          <div className="search-wrap" style={{ flex:1,maxWidth:480 }}>
            ...
            <input
              ...
              placeholder="Buscar por nombre, NIT, ciudad o categoría..."
                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        <div className="actions-group">
                          ...
                          <button className="btn-accion btn-accion-eliminar" onClick={() => openDeleteTarget(p)} title="Stop">
