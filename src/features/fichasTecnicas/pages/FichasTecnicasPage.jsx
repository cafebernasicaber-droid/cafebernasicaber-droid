...
function ModalFichaForm({ fichaInicial, onSave, onClose }) {
  ...
  // Reutiliza la misma lista de insumos ya cargada arriba — solo se filtra
  // por la categoría especial de vasos y por estado Activo (los inactivos
  // no deben poder seleccionarse).
  // Un "vaso" ya no es una categoría especial: es cualquier insumo cuya
  // unidad de medida sea "oz". Se reutiliza la misma lista de insumos ya
  // cargada arriba, solo filtrando por unidad y estado Activo.
  const vasosDisponibles = insumos.filter(i => i.unidadMedida === 'oz' && i.estado === 'Activo');
  ...
  const handleSubmit = async e => {
    ...
    const data = { ...form, id_producto:Number(form.id_producto), porciones:Number(form.porciones), tiempo_prep:Number(form.tiempo_prep), costo_estimado:Number(form.costo_estimado), vaso_id:Number(form.vaso_id) };
