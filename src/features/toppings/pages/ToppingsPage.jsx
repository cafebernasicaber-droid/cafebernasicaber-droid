...
function ToppingModal({ inicial, onClose, onSave }) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const r = inicial
        ? await toppingsService.update(inicial.id, { ...form, gratuito: form.gratuito || false })
        : await toppingsService.create({ ...form, gratuito: form.gratuito || false });
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" style={{maxWidth:500,textAlign:'left',padding:'32px 36px'}} onClick={e=>e.stopPropagation()}>
        ...
        <p style={{fontSize:13,color:'var(--text-muted)',marginBottom:20}}>{inicial ? `Modificando: ${inicial.nombre}` : 'Agrega un topping al menú'}</p>
        ...
        <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:14}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
            ...
            <div>
              <label style={{fontSize:12,fontWeight:700,color:'var(--text-secondary)',display:'block',marginBottom:5}}>Precio (COP)</label>
              <input type="number" min="0" value={form.precio} onChange={set('precio')} placeholder="0 = gratis" style={{width:'100%',padding:'10px 12px',border:'1.5px solid var(--border)',borderRadius:8,fontSize:14,outline:'none'}}/>
            </div>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <label htmlFor="gratuito" style={{fontSize:13,fontWeight:600,color:'var(--text-secondary)'}}>Topping gratuito (no tiene costo adicional)</label>
          </div>
          <div>
            <label style={{fontSize:12,fontWeight:700,color:'var(--text-secondary)',display:'block',marginBottom:5}}>Descripción</label>
            <textarea value={form.descripcion} onChange={set('descripcion')} placeholder="Descripción opcional..." rows={2}
              style={{width:'100%',padding:'10px 12px',border:'1.5px solid var(--border)',borderRadius:8,fontSize:14,outline:'none',resize:'vertical'}}/>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                <tbody>
                  {displayed.map(t => (
                    <tr key={t.id}>
                      ...
                      <td style={{color:'var(--text-muted)',fontSize:13}}>{t.descripcion||'—'}</td>
                      <td>
